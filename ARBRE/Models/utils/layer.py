...
class FFN(nn.Module):
    ...
    def forward(self, origin, out):
        out = self.bn1((out + origin).permute(0, 2, 1)).permute(0, 2, 1)
        out = self.bn2((out + self.w1(out)).permute(0, 2, 1)).permute(0, 2, 1)
        return out
...
class Weights(nn.Module):
    ...
    def forward(self, scores):
        len_s = len(scores)
        batch_size = len(scores[0])
        scores = self.w(scores.reshape([len_s*batch_size, -1]))  # [4, 128, 512] -> [512, 512]
        scores = scores.reshape([len_s, batch_size, -1])  # [4, 128, 1]
        scores = scores.mean(dim=1, keepdim=True).mean(dim=2, keepdim=True)  # [4, 1, 1]
        scores = F.softmax(scores.reshape([len_s]))  # [4]
        return scores
